...
<div class="container-fluid mt-4">
    <h2 class="mb-4 text-center text-uppercase fw-bold">Pedidos en Cocina</h2>
    
    <div class="row" id="orders-container">
        @foreach($orders as $order)
            <div class="col-md-4 mb-3 order-card-wrapper" id="order-{{ $order->id }}">
                <div class="card h-100 border-2 order-card shadow-sm" data-id="{{ $order->id }}" data-time="{{ $order->created_at->toISOString() }}">
                    <div class="card-header d-flex justify-content-between align-items-center fw-bold text-white bg-dark">
                        <span>Mesa: {{ $order->table_number }}</span>
                        <span class="badge bg-secondary">#{{ $order->id }}</span>
                    </div>
                    <div class="card-body">
                        <p class="card-text fs-5 fw-semibold">{{ $order->details }}</p>
                        <hr>
                        <div class="d-flex justify-content-between align-items-center">
                            <span class="fw-bold fs-4 timer-display">00:00</span>
                            
                            <button class="btn btn-success btn-lg fw-bold btn-complete" onclick="completeOrder({{ $order->id }})">
                                ✓ Atendido
                            </button>
                        </div>
                    </div>
